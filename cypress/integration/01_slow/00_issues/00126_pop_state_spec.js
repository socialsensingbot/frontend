...
describe('#126 Pop state : https://github.com/socialsensingbot/frontend/issues/126',
         function () {
           ...
           it('Reproduce Issue', () => {
               const url = MAP_URL + "?selected=greater%20london&max_time=1591100940000&min_time=1591007520000&active_number=exceedance&active_polygon=county";
               const slideLeftUrl = MAP_URL + "?selected=greater%20london&max_time=1591100940000&min_time=1590901260000&active_number=exceedance&active_polygon=county";
               cy.visitAndWait(url);
             ...
             cy.go('back');
               cy.wait(4000);
               cy.url({timeout: 20000}).should("equal",
                                               MAP_URL + "?selected=greater%20london&max_time=1591100940000&min_time=1590937260000&active_number=exceedance&active_polygon=county");
