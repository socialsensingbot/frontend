...
export class MapComponent {
  ...
  onMapReady(map: Map) {
    ...
    Storage.get("county_stats.json")
           ...
           .then(() => Storage.get("coarse_stats.json")
                              .then((url: any) =>
                                      fetch(url.toString())
                                        .then(response => response.json())
                                        .then(json => {
           ...
           .then(() => Storage.get("fine_stats.json")
                              .then((url: any) =>
                                      fetch(url.toString())
                                        .then(response => response.json())
                                        .then(json => {
  ...
  private main(map:Map) {
    ...
    class LegendControl extends Control {
      ...
      update() { this._div.innerHTML = makeLegend(colorData[active_number].values, colorData[active_number].colors, colorFunctions[active_number].getColor); };
      onAdd (map) {
        this.update();
        return this._div;
      };
     ...
     const read_data= ()=>{
      this.loadData()
        .then((tweet_json) =>{
          ...
          processData(this.tweetInfo, processedTweetInfo, polygonData,  this.stats, B, time_keys.slice(-default_max, -default_min), grid_sizes);
          resetLayers(false);
          tinfo.update_header();
          initSlider();
        }).catch(function(e) {
        ...
        $( ()=> {
          ($( ".timeslider" ) as any).slider({
                                      ...
                                      slide: ( event, ui ) =>{
                                        ...
                                        processData(this.tweetInfo, processedTweetInfo, polygonData, this.stats, B, time_keys.slice(-ui.values[ 1 ], -ui.values[ 0 ]), grid_sizes );
                                        resetLayers(false);
                                        tinfo.update_header();
